...
# ros
import rospy
from sawyer_reacher_clutter_v4 import sawyer_env # reacher in clutter
...
parser.add_argument('--model_lr',   type=float, default=3e-4) #3e-4 for sim # was 3e-3 for orig paper
parser.add_argument('--policy_lr',  type=float, default=3e-4) #3e-4 for sim # was 3e-3 for orig paper
...
if __name__ == '__main__':
    try:
        ...
        env_name = 'reacher_simparams_nodone'
        now = datetime.now()
        date_str = now.strftime("%Y-%m-%d_%H-%M-%S")

        path = './data/sawyer_clutter_200steps_10H/sac/2021-06-22_12-44-17_clutter_200steps_10H/'

        if os.path.exists(path) is False:
            os.makedirs(path)

        action_dim = env.action_dim  # 2 (reacher), 2 (pusher), 3 (shape sorter)
        state_dim  = env.state_dim  # 2 (reacher), 4 (pusher), 9 (shape sorter)
        hidden_dim = 128
        ...
        # load stuff
        test_frame='final'
        if args.mode == "hybrid" or args.mode=="sac":
            policy_net.load_state_dict(torch.load(path+'policy_{}.pt'.format(test_frame), map_location='cpu'))
        if args.mode == "hybrid" or args.mode=="mpc":
            model.load_state_dict(torch.load(path+'model_{}.pt'.format(test_frame), map_location='cpu'))
        rewards = pickle.load(open(path + 'reward_data.pkl', 'rb'))
        success = pickle.load(open(path + 'success_data.pkl', 'rb'))
        last_row = success[-1]
        frame_idx = last_row[0]
        ep_num = last_row[1]

        if (args.mode == "sac") or (args.mode == "hybrid"):
            temp_replay_buffer = pickle.load(open(path + 'replay_buffer.pkl', 'rb'))
            replay_buffer.buffer = temp_replay_buffer
            replay_buffer.position = len(replay_buffer)
        if (args.mode == "mpc") or (args.mode == "hybrid"):
            temp_replay_buffer = pickle.load(open(path + 'model_replay_buffer.pkl', 'rb'))
            model_replay_buffer.buffer = temp_replay_buffer
            model_replay_buffer.position = len(replay_buffer)
        ...
        while (frame_idx < max_frames):# and (ep_num < 101):
            state = env.reset()
            if state is None:
                break
            if args.mode == "sac":
